...
public class InterviewerController : MonoBehaviour
{
    ...
    [Header("Mood Badge")]
    [SerializeField] private TextMeshProUGUI moodBadgeText;
    [SerializeField] private RawImage moodBadgeDot;
    [SerializeField] private CanvasGroup moodBadgeCanvasGroup;
    ...
    private void UpdateMoodBadge(InterviewerMood mood)
    {
        if (moodBadgeText != null && MoodLabels.TryGetValue(mood, out string label))
            moodBadgeText.text = label;

        if (moodBadgeCanvasGroup != null)
        {
            if (moodBadgeFadeRoutine != null) StopCoroutine(moodBadgeFadeRoutine);
            moodBadgeFadeRoutine = StartCoroutine(BadgeFadeIn());
        }
    }

    private Coroutine moodBadgeFadeRoutine;

    private IEnumerator BadgeFadeIn()
    {
        moodBadgeCanvasGroup.alpha = 0f;
        float t = 0f;
        while (t < 1f)
        {
            t += Time.deltaTime / 0.25f;   // 0.25초 페이드 인
            moodBadgeCanvasGroup.alpha = Mathf.Clamp01(t);
            yield return null;
        }
    }
}